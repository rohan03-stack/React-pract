...
export default function Navbar() {
  ...
  return (
    <header className=' top-0 left-0 w-full z-20 text-white' data-aos='fade-up' data-aos-delay='300'>
      <div className='container mx-auto flex items-center justify-between p-5'>
        ...
        <div>
          ...
          {NavbarLinks.map((link) => (
            <a key={link.id} href={link.link} className='hover:text-gray-200 mr-8 text-lg' >
              {link.name}
            </a>
          ))}
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} sm:pt-2 md:hidden bg-[#801b9c] absolute top-0 left-0 w-full h-screen flex flex-col items-center justify-center space-y-8 pt-2`}>
